...
// Custom command for drag and drop file upload testing
Cypress.Commands.add(
  'dragAndDropFile',
  { prevSubject: true },
  (subject, fileName, options = {}) => {
    ...
    return cy
      .wrap(subject)
      .should('be.visible')
      .attachFile(fileName, defaultOptions);
  },
);

// Custom command for testing object widget file uploads
Cypress.Commands.add('uploadToObjectWidget', (blockType, fileName) => {
  // Wait for the block to be created
  cy.get(`.block.${blockType}`).should('be.visible');

  // Find the dropzone area and upload file
  cy.get(`.block.${blockType} .dropzone`)
    .should('be.visible')
    .dragAndDropFile(fileName);

  // Wait for upload to complete - look for either preview or success indicator
  cy.get(`.block.${blockType}`, { timeout: 15000 }).should(($block) => {
    // Check if upload completed by looking for preview or uploaded state
    expect($block.find('.preview, .uploaded, img').length).to.be.greaterThan(0);
  });
});
